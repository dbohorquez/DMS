...
<?php if(isset($_POST['bt-delete'])) list($warning, $success) = delete($_POST);?>
            ...
            <li><label for="donorId">Número de Identificación:</label><input type="text" class="text autocomplete" id="donorId" />
                <a href="javascript:void(0);" class="btn" onclick="var href = 'includes/forms/donationpromiseAdd.php?d=' + $('#donorId').attr('value'); $.colorbox({href:href});">Agregar Donación Virtual</a></li>
			<?php } ?>
            </ul>
        </div>
        <div class="column c50p last">
            <ul class="toolbar">
                ...
                <a href="javascript:void(0);" class="btn" onclick="var href = 'includes/forms/donationpromiseEdit.php?e=' + $('#sequence').attr('value'); $.colorbox({href:href});">Editar Donación Virtual</a></li>
			<?php } ?>
            </ul>
        </div>
            ...
            <table cellpadding="0" cellspacing="0"><thead>
                ...
                <?php
					$donations = getTable('donations d,companies w','d.deletedAt IS NULL and d.type=3 and w.id=companies_id','sequence asc');
